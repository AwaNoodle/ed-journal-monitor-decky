...
The frontend SHALL display upload counts and last-upload time received from backend events.
...
#### Scenario: Last upload time
- **WHEN** the backend emits an `upload_success` event
- **THEN** the panel SHALL update the "Last upload" timestamp display
