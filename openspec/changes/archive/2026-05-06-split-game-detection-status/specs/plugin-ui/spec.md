...
### Requirement: Display plugin status panel (single status line)
**Reason**: Replaced by two independent status fields (ED Status and Journal Status) that separate game detection from journal/watcher state.
**Migration**: The single `getStatusText()` function and its Field component are replaced by two separate Field components for "ED Status" and "Journal Status".
