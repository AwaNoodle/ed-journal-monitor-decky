...
- **THEN** every target's success count SHALL be set to 0
- **THEN** every target's fail count SHALL be set to 0
...
- **THEN** a `status_update` event SHALL be emitted to the frontend with the zeroed per-target statistics
