...
Tests FSSSignalDiscovered event batching, flush triggers, and signal extraction.
"""
...
class TestAddSignal:
    ...
    def test_strips_timestamp_from_signal(self, batcher):
        """Timestamp must not appear in individual signals (moved to message level)."""
        event = _make_signal_event()
        batcher.add_signal(event)

        batch_data = batcher.flush()
        assert "timestamp" not in batch_data["signals"][0]
    ...
    def test_preserves_uss_type(self, batcher):
        event = _make_signal_event(USSType="$USS_Type_Debris;")
        batcher.add_signal(event)
